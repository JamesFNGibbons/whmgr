...
<title>WHMGR - Filemanager</title>
<?php net2ftp("printJavascript"); ?>
<?php net2ftp("printCss"); ?>
</head>
<body onload="<?php net2ftp("printBodyOnload"); ?>">

<?php if(isset($_GET['whmgr_login_request'])): ?>
	<form id="LoginForm1" action="/filemanager/index.php" method="post" onsubmit="return CheckInput(this);">
									<fieldset>
										<div>
											<label>FTP server</label>
												<input type="text" name="ftpserver" value="localhost" class="form-poshytip" title="Example: ftp.server.com, 192.123.45.67">
									<input type="text" name="ftpserverport" value="21" style="position: absolute; left: 370px; width: 45px;" maxlength="5" class="form-poshytip" title="Enter the FTP server port (21 for FTP, 22 for FTP SSH or 990 for FTP SSL) - if you're not sure leave it to 21">
										...
										<div>
											...
											<input type="text" name="username" value="<?php print $_GET['username']; ?>" class="form-poshytip" title="Enter your username">
										</div>

										<div>
											<label>Password</label>
											<input type="password" name="password" value="<?php print $_GET['password']; ?>" class="form-poshytip" title="Enter your password">
										</div>
...
<script>document.getElementById('LoginForm1').submit();</script>
<?php else: ?>
